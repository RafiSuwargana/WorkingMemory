/**
 * Capacity Test Image Preloader
 * Preloads all capacity test images to prevent loading delays during test
 */

class CapacityPreloader {
    constructor() {
        this.images = [];
        this.loadedCount = 0;
        this.totalCount = 0;
    ...
    /**
     * Generate list of all possible capacity test images
     * Based on the image files available (0.png to 30.png)
     */
    generateImageList() {
        const imageNumbers = [];
        // Add all numbers from 0 to 30 (based on the capacity folder contents)
        for (let i = 0; i <= 30; i++) {
            imageNumbers.push(i);
        }
        
        return imageNumbers.map(num => `/images/capacity/${num}.png`);
    }
    ...
    preloadImage(src) {
        return new Promise((resolve) => {
            ...
            const handleError = () => {
                console.warn(`Failed to load capacity image: ${src}`);
                this.loadedCount++;
                this.images.push({
                    src: src,
                    element: null,
    ...
    /**
     * Start preloading all capacity test images
    ...
    async preloadAll() {
        ...
        console.log(`Starting to preload ${this.totalCount} capacity images...`);
        ...
        try {
            ...
            console.log('Capacity image preloading complete!');
            return this.images;
        } catch (error) {
            console.error('Error during capacity image preloading:', error);
            throw error;
        }
    }
...
window.capacityPreloader = new CapacityPreloader();

// Auto-start preloading when script loads
document.addEventListener('DOMContentLoaded', function() {
    console.log('Capacity preloader initialized');
});