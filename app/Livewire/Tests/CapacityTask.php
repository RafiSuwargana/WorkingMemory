...
class CapacityTask extends Component
{
    ...
    public $totalTrials = 50; // Jumlah total trial
    ...
    public function handleImageClick($imageIndex)
    {
        if (!$this->isTesting || $this->answered) {
            return;
        }

        // Toggle selection
        if (in_array($imageIndex, $this->userAnswers)) {
            $this->userAnswers = array_diff($this->userAnswers, [$imageIndex]);
        } else {
            $this->userAnswers[] = $imageIndex;
        }

        // Auto submit when correct number of images selected
        $expectedAnswers = count($this->correctAnswers);
        if (count($this->userAnswers) == $expectedAnswers) {
            $this->submitAnswer();
        }
    }
    ...
    public function submitAnswer()
    {
        ...
        $this->js('setTimeout(() => { $wire.hideFeedbackAndProceed() }, 2000);');
    }
    ...
    public function completeTest()
    {
        ...
        $this->accuracy = $this->totalCorrect / $this->totalTrials * 100;

        // Update test session dengan summary
        $this->testSession->update([
            'status' => 'completed',
            'completed_at' => now(),
            'total_questions' => $this->totalTrials,
            'correct_answers' => $this->totalCorrect,
            'wrong_answers' => $this->totalTrials - $this->totalCorrect,
        ]);
    }
